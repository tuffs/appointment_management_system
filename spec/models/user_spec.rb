...
RSpec.describe User, type: :model do
  it 'is valid with valid attributes' do
    user = User.new(name: 'John Doe', email: 'john@doe.com', password: 'p@ssword1')
    expect(user).to be_valid
  end

  it 'is invalid without a name' do
    user = User.new(name: nil, email: 'john@doe.com', password: 'p@ssword1')
    expect(user).to_not be_valid
  end

  it 'is invalid wihout an email' do
    user = User.new(name: 'John Doe', email: nil, password: 'p@ssword1')
    expect(user).to_not be_valid
  end

  it 'is invalid with a duplicate email address' do
    User.create!(name: 'Jane Doe', email: 'jane@example.com', password: 'p@ssword1')
    duplicate_user = User.new(name: 'John Doe', email: 'jane@example.com', password: 'p@ssword1')
  ...
  it 'encrypts the password when saving' do
    user = User.create!(name: 'John Doe', email: 'john@doe.com', password: 'p@ssword1')
  ...
  it 'requires a password to be at least 8 characters long, include at least one number, and one special character' do
    user = User.new(name: 'John Doe', email: 'john@doe.com', password: 'short@1')
    expect(user).to_not be_valid
